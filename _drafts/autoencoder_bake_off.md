...
I wanted to know if a stacked autoencoder is better than a sparse on for anomaly detection or if a variational autoencoder learns better features for classification than a vector-quantized one.
...
From that we will try to form a few hypothesis on how they will perform on the tasks.
...
Again, we have no restrictions on the latent space, but the encoding is expected to be worse though the greedy training procedure.

### Sparse Autoencoder

The sparse autoencoder imposes a sparsity constraint on the latent code.
Each element in the latent code should only active with a probability $$p$$.
...
The divergence is averaged over the batch size.
...
The denoising one is in the habit of vanishing some thin lines.
...
The most meaninful sampls were generated by the strict beta-VAE.
...
Then again, it's encoder outputs are less similar to standard Gaussian samples, why it fails to assign the unconditioned samples any meaning most of the time.
...
Unfortunately, conceptualizing a 20-dimensional space in your mind is not a tasks humans were designed for at all, but there are tools at our disposal that let us overcome this hurdle.
...
On the picture below, you can see a scatter plot for each of our autoencoders latent spaces.
Each point is the latent code for one image and the color represents the digit in the image.
...
There are some obvious similarities to the plots.
...
While the cluster of ones is nicely separated, the other digit's clusters are mixing at their borders.
This means that the shallow autoencoder maps some instances of different digits to the same latent code, resulting in weird results we have seen in the reconstructions.
...
The strict beta-VAE uses the least space because the divergence loss is enforces the most.

We can explain the interpolation behavior of the VAEs, too.
The VAE and strict beta-VAE produce somthing looking like a three during the interpolation, while the loose beta-VAE does not.
For both of the former, the clusters of two and three are neighbors and are completely separated for the latter.
...
Ok, so looking at generated images and latent spaces is nice and all, but what if I want to do something productive with an autoencoder?
...
Unsurprisingly, next to all autoencoders improved on the baseline, with the sparse one being the exception.
...
So, why is that?
How are the two autoencoders that trailed behind in next to all other tasks so good at anomaly detection?
...
If our latent space generalizes to well, we are hurting our anomaly detection performance.

The ability of an autoencoder to generalize is dependent on its capacity and the dimensionality of the latent space.
We can lower the dimensions of the latent space, but only so much (two in this case).
The shallow autoencoder struck the right balance of how well it modeled the training data, because of its limited capacity.
The stacked autoencoders greedy layer-wise training is limiting its ability generalize in a similar way.
...
As I said in the beginning, this article cannot draw any conclusions beyond MNIST, but it serves as a nice pointer, nevertheless.
It would be nice to do another run with a more complex dataset or one where the sparse autoencoder had it's time to shine.
